  ...
  useEffect(() => {
    if (project) {
      setProjectData({
        ...
        gradeLevel: project.gradeLevel || '5-6',
        subjectArea: project.subjectArea || '',
        reflectionPrompts: project.reflectionPrompts || [],
        ethicalFlags: project.ethicalFlags || [],
      ...
      <DialogContent className={`sm:max-w-4xl max-h-[90vh] overflow-y-auto transition-all duration-300 ${showEvaluation ? 'max-w-6xl' : ''}`}>
        <DialogHeader>
          <DialogTitle className="text-xl font-bold text-gray-900 flex items-center gap-2">
            ...
            {isEditing ? 'Edit Project Goals' : 'Project Goals'}
          </DialogTitle>
          <DialogDescription className="text-gray-600">
            {isEditing 
              ? 'Update your project goals, constraints, and success criteria.'
              : 'Review your project goals, constraints, and success criteria.'
            }
          </DialogDescription>
        </DialogHeader>
                    ...
                    <div className="flex items-center gap-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`text-sm font-medium ${isEvaluationPassing() ? 'text-green-800' : 'text-curiosity-coral'}`}>
                          {isEvaluationPassing() ? 'Ready to Save' : 'Needs Improvement'}
                        </span>
                      </div>
            ...
            <div className="flex gap-3">
              ...
              {isEditing && (
                ...
                >
                  ...
                  {showEvaluation && !isEvaluationPassing() ? 'Improve Project' : 'Save Changes'}
                </Button>
              )}
            </div>
          </div>
