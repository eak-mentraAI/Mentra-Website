...
const faqSections = [
  ...
  },
  {
    ...
    questions: [
      ...
      { question: "Does the AI decide what happens to my child?", answer: "No. Two kinds of intelligence work together. Deterministic engines decide what a child has truly demonstrated and who needs a teacher — by rules, not guesses, so the result is provable and the same every time. AI only explains those decisions in plain language. And a human always has the final say. Engine decides, AI explains, human approves — the machine never acts on a child unsupervised." },
      { question: "What does Mentra do when it isn't sure?", answer: "It says so. Mentra is built to refuse to lie — when the evidence is thin it tells you \"we don't know yet\" or \"this will fill in as they work\" instead of inventing a score or a fabricated extreme. And it never mistakes a child parroting an answer for genuine mastery." },
      { question: "Could Mentra ever fake a mastery score?", answer: "We treat that as the worst thing our product could do to a kid — so we built an automated gate that blocks the AI from writing anything except plain-language explanations. Every actual judgment about what a student has learned comes from deterministic rules, not a language model guessing. We've tested this against a brand-new student account with zero history: no shortcuts, no defaults, no invented \"progressing\" — just an honest \"not enough evidence yet.\"" },
  ...
  },
  {
    ...
    questions: [
      ...
      { question: "What is the ontology and knowledge graph you talk about?", answer: "As a student works, Mentra captures learning signals and maps them into a per-child knowledge graph — a living model of one mind, where concepts connect to what they unlock and struggles connect to their root causes. The ontology is the governed layer in between: it gives every signal a precise meaning, decides whether AI is even allowed to reason on it, and acts as the safety boundary that lets cognition data flow to partners without ambiguity. Every child's graph is sealed in their own tenant." },
